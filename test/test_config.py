...
def test_tov_words(rc_extra, verbose=False):
    s, g = check("Ég er með bankareikning og góðan þjónustufulltrúa.", rc_extra)
    assert len(s) == 37
    errors = {4, 7}
    ...
    s, g = check("Fylltu út þá fjárhæð sem á að leggja inn á debitkortið.", rc_extra)
    assert len(s) == 54
    errors = {4, 11}
    for ix in range(len(g)):
        print(ix)
...
def test_capitalization(rc_extra, verbose=False):
    s, g = check("Þú getur gert þetta í Fyrirtækjaþjónustunni okkar.", rc_extra)
    assert len(s) == 50
    errors = {6}
    for ix in range(len(g)):
        print(ix)
        if ix in errors:
            assert g[ix].error_code == "Z001"
        else:
            assert not g[ix].error_code
...
if __name__ == "__main__":
    ...
    test_capitalization(r, verbose=True)
